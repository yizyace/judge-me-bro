...
**Spec label:** `judging-schemas`

This doc is the **single source of truth** for the shapes below. Cite any
contract from another spec by its label, `judging-schemas#<SchemaName>` (e.g.
`judging-schemas#IdeaEvaluationOutput`). Reference by schema *name*, not by
§number — names are stable, section numbers may shift.

| Label | Kind | Defined in | Purpose |
|-------|------|-----------|---------|
| `judging-schemas#Slug` | type | [§1.2](#12-common-field-types) | kebab-case identifier |
| `judging-schemas#Version` | type | [§1.2](#12-common-field-types) | persona distillation version (int) |
| `judging-schemas#Score10` | type | [§1.2](#12-common-field-types) | integer score, 1–10 |
| `judging-schemas#Unit` | type | [§1.2](#12-common-field-types) | number in [0, 1] |
| `judging-schemas#EvaluatorVersion` | type | [§1.2](#12-common-field-types) | `eval@vN` scoring-math tag |
| `judging-schemas#IsoTimestamp` | type | [§1.2](#12-common-field-types) | ISO-8601 with offset |
| `judging-schemas#RunId` | type | [§1.2](#12-common-field-types) | shared id for one pipeline pass |
| `judging-schemas#RubricWeights` | type | [§1.2](#12-common-field-types) | 5 criteria → weight, sums to 1.0 |
| `judging-schemas#JudgeRef` | object | [§1.3](#13-judgeref) | pointer to a distilled judge snapshot |
| `judging-schemas#IdeaEvaluationOutput` | object | [§2.2](#22-schema-a--ideaevaluationoutput-what-the-judge-returns) | Phase 1 — what the idea-judge returns (validation gate) |
| `judging-schemas#IdeaEvaluation` | object | [§2.3](#23-schema-b--ideaevaluation-persisted-record) | Phase 1 — persisted record (output + provenance + weighted total) |
| `judging-schemas#MetaEvaluationOutput` | object | [§3.3](#33-schema-a--metaevaluationoutput-what-the-rater-returns) | Phase 2 — what the meta-judge returns (validation gate) |
| `judging-schemas#MetaEvaluation` | object | [§3.4](#34-schema-b--metaevaluation-persisted-record) | Phase 2 — persisted record (output + provenance + meta score) |

**Referencing from other specs** — pick per context:

- *Schema label* — `judging-schemas#IdeaEvaluation` when you mean **this exact
  contract** (a field shape).
- *Section pointer* — `judging-schemas §2` when you mean **this whole step**
  (procedure + prompt + validation).

When another spec copies one of these JSON examples, link back to the label so
the two stay in sync. Changing a shape here is a breaking change for every spec
that cites it — bump the relevant `EvaluatorVersion` (see [§3.7](#37-meta-score-math-evaluator-v1)) when the
math changes, and call out schema changes in the consumer specs.
...
**Label:** `judging-schemas#JudgeRef`
...
**Label:** `judging-schemas#IdeaEvaluationOutput`
...
**Label:** `judging-schemas#IdeaEvaluation`
...
**Label:** `judging-schemas#MetaEvaluationOutput`
...
**Label:** `judging-schemas#MetaEvaluation`
